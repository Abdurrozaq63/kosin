...
import { Home, MapPin, Ruler, BedDouble, Phone, Wallet } from 'lucide-react';

export default function Kos() {
  const router = useRouter();
  const [kosPemilik, setKosPemilik] = useState<any[]>([]);
  const [selectedKos, setSelectedKos] = useState<any | null>(null);
  ...
  const [dataKos, setDataKos] = useState<any | null>(null);
  const { idStore } = useIdStore();

  useEffect(() => {
    fetchIdkos();
    const kosin = KosStore.find((k) => k.id_kos === idStore);
    setDataKos(kosin);
    console.log('fetchkosin', idStore);
  }, []);

  useEffect(() => {
    fetchKoss();
    console.log('fetchkos');
  }, []);
  ...
  function handleEdit(kos: any) {
    setSelectedKos(kos);
    setOpenModal('edit');
  }
  function handleDelete(id_tipe: string) {
